    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="手机号码" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入手机号码" :disabled="disabledAuth('user:form:phone')"
  ...
  export default {
    ...
    data() {
      return {
        ...
        picUrl: "",
        url: {
          add: "/sys/user/add",//添加用户方法
          edit: "/sys/user/edit",//保存用户方法
          addUDepartIds: "/sys/user/addUDepartIds", // 引入为用户添加部门信息需要的url
          editUDepartIds: "/sys/user/editUDepartIds", // 引入为用户更新部门信息需要的url
          fileUpload: window._CONFIG['domianURL'] + "/sys/file/upload",
          imgerver: window._CONFIG['domianURL'] + "/sys/file/view",
    ...
    methods: {
        ...
        getAction(that.url.userWithDepart, {userId: that.userId}).then((res) => {
          ...
          if (res.success) {
            for (let i = 0; i < res.result.length; i++) {
              this.checkedDepartNames = res.result[i].departName;
              this.selectedDepartKeys = res.result[i].id;
            }
          } else {
            console.log(res.message);
          }
      ...
      handleSubmit() {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            formData.departId = this.selectedDepartKeys;
            let httpurl = '';
            let method = '';
            if (!this.model.id) {
      ...
      addDepartsToUser(that, formData) {
        ...
        console.log(this.userDepartModel)
      ...
      // 搜索用户对应的部门API
      onSearch() {
        this.$refs.departWindow.add(this.selectedDepartKeys, this.userId);
      },
      ...
      modalFormOk(formData) {
        ...
        this.selectedDepartKeys = formData.departList.key;
        this.checkedDepartNames = formData.departList.title
